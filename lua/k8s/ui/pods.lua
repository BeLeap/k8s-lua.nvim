    ...
    entry_modifier = function(picker_buffer, index, object)
      ...
      for _, condition in ipairs(conditions) do
        if condition.type == "ContainersReady" and condition.status == "False" then
          picker_buffer:highlight(global_contexts.ns_id, "K8sPodUnhealthy", { index - 1, 0 }, { index - 1, -1 })
        end
      end
    end,
    additional_keymaps = {
          ...
          return function()
            ...
            for k, v in pairs(logs) do
              local LogBuffer = buffer:new(
                "k8s://" .. pods:build_fqdn(object.metadata.namespace) .. "/" .. object.metadata.name .. "/log/" .. k
              )
